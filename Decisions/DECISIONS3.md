...
## 0. GPU usage — clarified for this phase
**Question raised:** Since training is happening on an M2 MacBook Air with
MPS GPU support, should baseline models use GPU?
**Clarified:** scikit-learn (Linear Regression, Random Forest) is CPU-only
by design, on any machine — not a Mac-specific limitation. XGBoost supports
GPU only via CUDA (NVIDIA), which MPS does not provide, so XGBoost will also
run on CPU here. GPU (via PyTorch's MPS backend, already set up in
src/device.py) becomes relevant starting in the sequence-model phase
(LSTM/CNN), not for these baseline models. Dataset size (~16K training rows)
is small enough that CPU training for these models will be fast regardless.
